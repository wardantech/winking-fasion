...
<div style="max-width:400px;margin:0 auto">
    ...
    <div id="receipt-data">
        <div class="centered">
            ...
            <p>{{trans('file.Address')}}: {{ $lims_biller_data->address }} {{ $lims_biller_data->city }}
                <br>{{trans('file.Phone Number')}}: {{ $lims_biller_data->phone_number }}
            </p>
        </div>
        ...
        <table>
            <tbody>
                ...
                @foreach($lims_service_sale_data as $service_sale_data)
                <?php
                    $lims_service_data = \App\Service::find($service_sale_data->product_id);

                    $service_name = $lims_service_data->name;
                ?>
                <tr>
                    <td colspan="2">
                        {{$service_name}}
                        <br>{{$service_sale_data->qty}} x {{number_format((float)($service_sale_data->total / $service_sale_data->qty), 2, '.', '')}}

                        @if($service_sale_data->tax_rate)
                            <?php $total_product_tax += $service_sale_data->tax ?>
                            [{{trans('file.Tax')}} ({{$service_sale_data->tax_rate}}%): {{$service_sale_data->tax}}]
                        @endif
                    </td>
                    <td style="text-align:right;vertical-align:bottom">{{number_format((float)$service_sale_data->total, 2, '.', '')}}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
